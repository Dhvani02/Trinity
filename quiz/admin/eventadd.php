...
?>
  <div class="container" style="margin-top:25px;">
    ...
    <li>
      <div class="collapsible-header <?php if(!isset($_POST['title'])){  echo 'active';}?>"><i class="material-icons">filter_drama</i>Event Add Portal</div>
          ...
          <form class="col s12" action="eventadd.php" method="POST"  enctype="multipart/form-data">
            <div class="row">
			  ...
			  <div class="input field col s12">
                ...
                <input type="date" class="datepicker" name="edate">
			  </div>	
			  <div class="col s12">
                <br><br>
			  </div>				  
...
?>			
	...
	<div class="row">
	  <div class="col s12" >
    <li>
      <div class="collapsible-header"><i class="material-icons">filter_drama</i>Add Result</div>	  
		 
 <?php
 if(isset($_POST['title']) && isset($_POST['desc']) && isset($_POST['shouri']) && isset($_POST['smini']) && isset($_POST['ehouri']) && isset($_POST['emini']) && isset($_POST['venue']) && isset($_POST['file']) && isset($_POST['edate']) && isset($_POST['cat']))
 {
	 $title=$_POST['title'];
	 $desc=$_POST['desc'];
	 $shour=$_POST['shouri'];
	 ...
	 $edate=$_POST['edate'];
	 $cat=$_POST['cat'];
	 if(!empty($title) && !empty($desc) && !empty($shour) && !empty($smin) && !empty($ehour) && !empty($emin) && !empty($venue) && !empty($file) && !empty($edate) && !empty($cat))
	 {
		 $stime=$shour.':'.$smin.':'.'00';
		 $etime=$ehour.':'.$emin.':'.'00';		 
		 $query="INSERT INTO event VALUES('','".mysql_real_escape_string($title)."','".mysql_real_escape_string($desc)."','".mysql_real_escape_string($edate)."','".mysql_real_escape_string($stime)."','".mysql_real_escape_string($etime)."','".mysql_real_escape_string($venue)."','".mysql_real_escape_string($cat)."')";
			 ...
			 if(isset($name))
			  {
				if(!empty($name))
				{
				   ...
				   if(($extension=='jpg' || $extension=='jpeg' || $extension=='png') && ($type=='image/jpeg' || $type=='image/png'))
					   ...
					   {
									 echo '
										  <ul class="collection l8 s12" style="opacity: 0.8;border:1px solid black;">
